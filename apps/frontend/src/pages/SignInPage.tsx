const STRAPI_URL = import.meta.env.VITE_STRAPI_URL

if (!STRAPI_URL) {
  throw new Error('VITE_STRAPI_URL is not defined')
}

export default function SignInPage() {
  const onGoogleSignIn = () => {
    // Use Strapi's OAuth without redirect parameter
    const url = `${STRAPI_URL}/api/connect/google`
    window.location.href = url
  }

  return (
    <main className="min-h-screen flex items-center justify-center p-6">
      <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-8 max-w-md w-full space-y-8 fade-in">
        <div className="text-center space-y-4">
          <img
            src="/TOAST_MAN_LOGO.jpeg"
            alt="Toast Man"
            className="w-32 h-32 rounded-full object-cover mx-auto shadow-md ring-4 ring-primary/20"
          />
          <div className="inline-block bg-gray-50 border border-gray-200 rounded-2xl rounded-bl-sm px-5 py-3">
            <p className="text-gray-900 font-extrabold text-xl">Hi, I'm Toast Man! 👋</p>
          </div>
          <h1 className="text-4xl font-black text-gray-900">
            Welcome to Toast<span className="text-primary">2</span>Host
          </h1>
          <p className="text-gray-600 text-lg leading-relaxed">
            Sign in to connect with your alumni network
          </p>
        </div>

        <button
          onClick={onGoogleSignIn}
          className="bg-primary text-black font-bold px-9 py-3 rounded-lg shadow-md hover:shadow-lg hover:bg-primary-dark transition-all w-full inline-flex items-center justify-center rounded-xl px-6 py-4 text-lg font-bold shadow-lg hover:shadow-xl transition-all group"
        >
          <svg className="w-6 h-6 mr-3" viewBox="0 0 24 24">
            <path fill="currentColor" d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z"/>
            <path fill="currentColor" d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z"/>
            <path fill="currentColor" d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l2.85-2.22.81-.62z"/>
            <path fill="currentColor" d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z"/>
          </svg>
          Continue with Google
        </button>

        <p className="text-center text-sm text-gray-500 leading-relaxed">
          By continuing, you agree to our{' '}
          <a href="/legal/terms" className="text-primary hover:underline font-medium">Terms</a>
          {' '}and{' '}
          <a href="/legal/privacy" className="text-primary hover:underline font-medium">Privacy Policy</a>
        </p>
      </div>
    </main>
  )
}
