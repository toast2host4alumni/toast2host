 ...
 * Run this after Strapi is running: node seeders/import-universities.js
 */
...
async function importUniversities() {
  ...
  if (!fs.existsSync(seedDataPath)) {
    console.error('Seed data file not found. Run seed-universities.js first!');
    process.exit(1);
  }

  const universities = JSON.parse(fs.readFileSync(seedDataPath, 'utf-8'));
  ...
  for (let i = 0; i < universities.length; i += BATCH_SIZE) {
    ...
    for (const university of batch) {
      try {
        ...
        errors++;
        if (errors <= 5) {
          console.error(`  Error importing ${university.name}:`, error.message);
        }
      }
    }
  }
